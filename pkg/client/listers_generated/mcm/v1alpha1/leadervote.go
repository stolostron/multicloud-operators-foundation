...
type LeaderVoteLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.LeaderVote, err error)
	// Get retrieves the LeaderVote from the index for a given name.
	Get(name string) (*v1alpha1.LeaderVote, error)
...
// List lists all LeaderVotes in the indexer.
func (s *leaderVoteLister) List(selector labels.Selector) (ret []*v1alpha1.LeaderVote, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.LeaderVote))
	})
	return ret, err
}

// Get retrieves the LeaderVote from the index for a given name.
func (s *leaderVoteLister) Get(name string) (*v1alpha1.LeaderVote, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("leadervote"), name)
	}
	return obj.(*v1alpha1.LeaderVote), nil
}
