...
type WorkLister interface {
	...
	List(selector labels.Selector) (ret []*mcm.Work, err error)
...
// List lists all Works in the indexer.
func (s *workLister) List(selector labels.Selector) (ret []*mcm.Work, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*mcm.Work))
	})
	return ret, err
}
...
type WorkNamespaceLister interface {
	...
	Get(name string) (*mcm.Work, error)
...
// List lists all Works in the indexer for a given namespace.
func (s workNamespaceLister) List(selector labels.Selector) (ret []*mcm.Work, err error) {
...
// Get retrieves the Work from the indexer for a given namespace and name.
func (s workNamespaceLister) Get(name string) (*mcm.Work, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(mcm.Resource("work"), name)
	}
	return obj.(*mcm.Work), nil
}
