...
type ClusterJoinRequestInformer interface {
	...
	Lister() internalversion.ClusterJoinRequestLister
}
...
func NewClusterJoinRequestInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredClusterJoinRequestInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Mcm().ClusterJoinRequests().List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Mcm().ClusterJoinRequests().Watch(options)
			},
		},
...
func (f *clusterJoinRequestInformer) defaultInformer(client internalclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *clusterJoinRequestInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&mcm.ClusterJoinRequest{}, f.defaultInformer)
}

func (f *clusterJoinRequestInformer) Lister() internalversion.ClusterJoinRequestLister {
	return internalversion.NewClusterJoinRequestLister(f.Informer().GetIndexer())
}
