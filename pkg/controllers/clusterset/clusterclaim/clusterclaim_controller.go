...
func (r *Reconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	originalClaim := clusterclaim.DeepCopy()
	...
	if clusterpool.Labels != nil && len(clusterpool.Labels[utils.ClusterSetLabel]) != 0 {
		...
		clusterclaim.Labels[utils.ClusterSetLabel] = clusterpool.Labels[utils.ClusterSetLabel]
	} else {
		delete(clusterclaim.Labels, utils.ClusterSetLabel)
	}

	if !reflect.DeepEqual(originalClaim.Labels, clusterclaim.Labels) {
