...
OCM_BRANCH=${OCM_BRANCH:-backplane-2.11}
...
oc apply -f https://raw.githubusercontent.com/stolostron/cluster-proxy/$OCM_BRANCH/charts/cluster-proxy/crds/managedproxyserviceresolvers.yaml
...
../$HELM install \
  ...
  --set global.namespace=open-cluster-management \
	...
	--set global.imageOverrides.cluster_proxy="${IMAGE_CLUSTER_PROXY}:backplane-2.11" \
