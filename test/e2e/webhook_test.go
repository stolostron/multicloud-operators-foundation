...
var _ = ginkgo.Describe("Testing webhook cert rotation", func() {
	...
	ginkgo.It("should create and update the managedCluster after cert rotation successfully", func() {
		...
		err = kubeClient.CoreV1().Secrets("open-cluster-management").Delete(context.TODO(), "foundation-webhook", metav1.DeleteOptions{})
		gomega.Expect(err).ShouldNot(gomega.HaveOccurred())

		gomega.Eventually(func() error {
			_, err := kubeClient.CoreV1().Secrets("open-cluster-management").Get(context.TODO(), "foundation-webhook", metav1.GetOptions{})
			return err
